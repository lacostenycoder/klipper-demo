$(function () {
  ...
  //https://www.kimonolabs.com/api/42tbt3m0?apikey=GYMKOJ7U9cdnoSw2ZMxxWmWJevyCp6AK
		...
		if(data.length){
			data.forEach(function (item) {
				if(item.id == index){
					...
					container.find('h3').text(item.img.txt);
					container.find('img').attr('src', item.img.src);
					container.find('p').text(item.img.text);
				}
			});
		}
